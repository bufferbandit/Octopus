...
if [ ! -z "${param_HasArguments}" ]; then
	param_HasArguments=$(sed 's/ /<SPACE>/g' <<< "$param_HasArguments")
...
echo -ne ${HeaderSize}${LinkCLSID}${LinkFlags}${FileAttributes}${CreationTime}${AccessTime}${WriteTime}${FileSize}${IconIndex}${ShowCommand}${Hotkey}${Reserved}${Reserved2}${Reserved3}${IDLIST}${TerminalID}${STRING_DATA} | sed "s/<SPACE>/ /g" > "${OUTPUT_FILE}"
